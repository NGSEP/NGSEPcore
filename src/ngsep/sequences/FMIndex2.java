 ...
 */
public class FMIndex2 implements Serializable {
	...
	private List<FMIndexSingleSequence2> internalIndexes = new ArrayList<>();
	 ...
	 * 
	 * @param sequences to add to the index. Each QualifiedSequence object in the
	 *                  list should have a name and its characters
	 */
	public void loadQualifiedSequenceList(QualifiedSequenceList sequences) {
		...
		for (QualifiedSequence seq : sequences) {
			...
			if (internalSequence.length() + next.length() > 1000000000) {
				System.out.println(
						"Building index for " + nI + " sequences. Total sequence length: " + internalSequence.length());
				long time = System.currentTimeMillis();
				FMIndexSingleSequence2 index = new FMIndexSingleSequence2(internalSequence);
	...
	public void loadUnnamedSequences(List<? extends CharSequence> sequences, int tally, int indexl) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (internalSequence.length() + next.length() > (100000000)) {
				System.out.println("		Building index for " + nI + " sequences. Total sequence length: "
						+ internalSequence.length());
				long time = System.currentTimeMillis();
				FMIndexSingleSequence2 index = new FMIndexSingleSequence2(internalSequence, tally, indexl);
				System.out.println("		Built index in " + (System.currentTimeMillis() - time) + " milliseconds");
				System.out.println("		-----------------");
				internalIndexes.add(index);
				internalMetadata.add(internalIdxMetadata);
	...
	public List<ReadAlignment> search(String searchSequence, int firstIndex, int lastIndex) {
		...
		for (int i = 0; i < internalIndexes.size(); i++) {
			FMIndexSingleSequence2 idxSeq = internalIndexes.get(i);
			CombinedMultisequenceFMIndexMetadata metadata = internalMetadata.get(i);
			if (!metadata.overlapWithIndexes(firstIndex, lastIndex))
				continue;
			...
			for (int internalPosMatch : matches) {
				...
				if (realData == null)
					continue;
				if (realData[0] >= sequenceLengths.size())
					throw new RuntimeException("Problem with internal index answer: " + realData[0] + "-" + realData[1]
							+ ". Absolute: " + internalPosMatch + " total length: " + metadata.getTotalLength()
							+ " first idx: " + metadata.getFirstInputSequenceIdx() + " last idx: "
							+ metadata.getLastInputSequenceIdx());
				int first = internalPosMatch - realData[1];
				int l = searchSequence.length();
				int last = first + l - 1;
				if (last >= sequenceLengths.get(realData[0]))
					continue;
				String seqName = "" + realData[0];
				if (sequencesWithNames != null)
					seqName = sequencesWithNames.get(realData[0]).getName();
