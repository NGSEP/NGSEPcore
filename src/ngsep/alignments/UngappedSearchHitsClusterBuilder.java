...
public class UngappedSearchHitsClusterBuilder {
	private int idxSubjectDebug = 4;
	private int queryLengthDebug = 31887;
	...
	private List<UngappedSearchHit> selectHitsByDistanceWithMedian(Map<Integer, List<UngappedSearchHit>> hitsMultiMap, int median, int maxDistance) {
		...
		for(List<UngappedSearchHit> hits:hitsMultiMap.values()) {
			...
			if(hit!=null) {
				if (debug) {
					System.out.println("Selected hits. Next qpos "+hit.getQueryIdx()+" hit: "+hit.getStart()+" estq: "+estimateQueryStart(hit)+" estS: "+estimateSubjectStart(hit)+" all starts: "+calculateHitStarts(hits));
				}
	...
	private List<UngappedSearchHit> removeDisorganized(List<UngappedSearchHit> selectedHits) {
		...
		if(n<=30) return selectSorted(selectedHits, 0, n-1);
